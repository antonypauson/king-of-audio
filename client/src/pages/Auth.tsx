...
const memeMap: { [key: string]: string } = {
...
const Auth: React.FC<AuthProps> = ({ onLoginSuccess }) => {
  ...
  const [currentMeme, setCurrentMeme] = useState<string>(memeMap["Default"]);
  ...
  const handleSignUp = async () => {
    try {
      ...
      setMessage("Sign up successful. Check our spam folder (since we can't afford a domain name right now) and verify your account");
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-center min-h-screen bg-background p-4 md:p-8 space-y-8 md:space-y-0 md:space-x-16"> 
      ...
      <div className="w-full md:w-2/5 p-5 md:py-20 md:px-10 bg-card rounded-lg shadow-lg"> {/* Reverted padding to original p-4 md:p-8 */}
